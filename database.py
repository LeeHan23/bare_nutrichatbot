...
class ApiClient(Base):
    ...
    documents = relationship(
        "DocumentMetadata", back_populates="client", cascade="all, delete-orphan"
    )
    patients = relationship(
        "Patient", back_populates="client", cascade="all, delete-orphan"
    )
...
class Patient(Base):
    ...
    medication_compliance = Column(
        String, nullable=True
    )  # FH-3.1.1.1 (good/variable/poor)
    activity_types = Column(JSON, default=list)  # FH-7.3.1.1
    extractor_food_allergies = Column(
        JSON, default=list
    )  # FH-1.6 (self-reported, extractor-filled)
    ...
    extractor_metadata = Column(
        JSON, default=dict
    )  # {field: {confidence, last_updated, source_session_id}}
    ...
    whatsapp_opted_out = Column(
        Boolean, default=False
    )  # True after patient replies STOP
...
def get_api_client_by_name(db_session, client_name: str):
    ...
    """
    return (
        db_session.query(ApiClient).filter(ApiClient.client_name == client_name).first()
    )
...
def get_client_documents(db_session, client_id: int):
    ...
    """
    return (
        db_session.query(DocumentMetadata)
        .filter(DocumentMetadata.client_id == client_id)
...
def get_document_by_id(db_session, document_id: int, client_id: int = None):
    ...
    """
    query = db_session.query(DocumentMetadata).filter(
...
class ContentMaterial(Base):
    ...
    )  # 3,5,7,14,21,30 (NULL for weekly EKA)
    content_type = Column(
        String(1), nullable=True, index=True
    )  # "E" | "K" | "A" | NULL (legacy nutrition)
    week_number = Column(
        Integer, nullable=True, index=True
    ...
    raw_tips = Column(
        JSON, default=list
    )  # [{tip_number,tip,source_hint}] or EKA structured dict
    ...
    )  # EKA: created_at + 14 days; NULL = never expires (legacy)
...
def normalise_phone_number(phone_number: str) -> str:
    ...
    cleaned = (
        phone_number.replace("whatsapp:", "").strip().replace(" ", "").replace("-", "")
    )
...
def get_patients_by_name(db_session, name: str, client_id: int):
    ...
    exact = (
        ...
        .filter(Patient.client_id == client_id, Patient.name.ilike(name.strip()))
        .all()
    )
    if exact:
        return exact
    return (
        db_session.query(Patient)
        .filter(Patient.client_id == client_id, Patient.name.ilike(f"%{name.strip()}%"))
        .all()
    )
...
def get_patient_by_ic(db_session, ic_number: str, client_id: int):
    ...
    patients = (
        db_session.query(Patient)
        .filter(Patient.client_id == client_id, Patient.ic_number.isnot(None))
        .all()
    )
    for p in patients:
...
def get_active_materials_for_conditions(
    db_session, condition_groups: list, day_offset: int
):
    """Find is_active=True materials matching any of the condition groups at this day offset."""
    return (
        db_session.query(ContentMaterial)
        .filter(
...
def get_delivery_log(
    db_session, patient_id: int = None, status: str = None, scheduled_date=None
):
    ...
    if scheduled_date:
        ...
        q = q.filter(
            ContentDeliveryLog.scheduled_date >= day_start,
...
def get_materials_by_filters(
    ...
    return (
        q.order_by(
            ContentMaterial.condition_group,
            ContentMaterial.content_type,
            ContentMaterial.week_number,
        )
        .offset(offset)
        .limit(limit)
        .all()
...
def get_weekly_feed_for_conditions(
    db_session,
...
def add_chat_message(
    db_session, session_id: str, patient_id: int | None, role: str, content: str
):
...
class CardiovascularScreening(Base):
    __tablename__ = "cardiovascular_screenings"

    id = Column(Integer, primary_key=True, autoincrement=True)
    patient_id = Column(
        String(255), index=True, nullable=False
    )  # Link to your existing patient structure
    centre_id = Column(String(255), nullable=False)  # NADI Centre ID
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Demographics (Protocol Section 16.1)
    age = Column(Integer, nullable=False)
    sex = Column(String(50), nullable=False)
    ethnicity = Column(String(100), nullable=False)
    postcode = Column(String(20), nullable=False)

    # Medical History (Protocol Section 16.1)
    has_diabetes = Column(Boolean, default=False, nullable=False)
    has_hypertension = Column(Boolean, default=False, nullable=False)
    has_dyslipidaemia = Column(Boolean, default=False, nullable=False)
    prior_cvd = Column(Boolean, default=False, nullable=False)

    # Medications (Protocol Section 16.1)
    on_bp_meds = Column(Boolean, default=False, nullable=False)
    on_diabetes_meds = Column(Boolean, default=False, nullable=False)
    on_statin = Column(Boolean, default=False, nullable=False)
    on_antiplatelet = Column(Boolean, default=False, nullable=False)

    # Lifestyle & Vitals (Protocol Section 16.1)
    is_smoker = Column(Boolean, default=False, nullable=False)
