...
10. ~~New, residual, low-priority: cases 14/16/17 personalization-only failures~~ — **investigated 2026-07-28/29**: individual re-runs confirmed 5 of that run's 9 failures were pure sampling noise (all passed on re-run), but 3 were reproducible root causes, now fixed in `rag.py`/`eval/test_rag.py`: L1 answers gave qualitative moderation language ("keep an eye on portion") without a concrete number, L3 answers implied care-team oversight without the literal phrase, and the L0 breakfast keyword check demanded 2 of 6 terms from a terse, word-budgeted answer. RAG suite improved 25/34 → **31/34**, the best result across all five full runs this session. Remaining 3 failures (all personalization) are two food-*category* (not single-dish) L1 questions still lacking a concrete number, plus one likely judge misclassification — see `docs/eval_and_roadmap.md` Part D and `eval/results/EVAL_REPORT.md` for full detail and next steps (case-34 judge re-check, weekly full-suite cron, DeepEval `G-Eval` migration still open).
