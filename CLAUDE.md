...
- `DATABASE_URL` — PostgreSQL URL for the app DB (users, api_clients, documents)
...
2. `rag.py` → `identify_target_disease()` (LLM call) → `get_rag_response()`
3. `vector_store.py` → `get_retriever(client_id)` builds a `MergedRetriever` combining:
   - **`base_knowledge`** collection (shared foundational knowledge)
   - **`client_{id}_knowledge`** collection (per-client isolated knowledge)
4. `chain_factory.py` → `create_conversational_chain()` wires an LCEL chain (LangChain 1.x): retriever → prompt → LLM → StrOutputParser, wrapped in `RunnableWithMessageHistory` for per-session memory
5. Streaming response via SSE with image tag parsing
...
| `database.py` | SQLAlchemy models: `ApiClient`, `DocumentMetadata`, `User` |
...
| `website_chat_router.py` | `POST /chat/get_response` streaming endpoint |
